...
let randAlgArray = [
    "R2' D2' U B2' D U2' R2' D2' F' L' D' R D R' U2' L' F U' L' B2'",
    "R2' D2' U B2' D U2' R2' D2' F' L' D' R D R' U2' L' F U' L' B2'",
    "R2' D2' U B2' D U2' R2' D2' F' L' D' R D R' U2' L' F U' L' B2'",
    "R2' D2' U B2' D U2' R2' D2' F' L' D' R D R' U2' L' F U' L' B2'",
]
...
let algoArrayPrime = [algoArray1,algoArray2,algoArray3,algoArray4];
let buttonTracker = [[],[],[],[]];

let partialSolve = '';
let startAlg = [randAlgArray[0],partialSolve].join(' ').trim();
let testAlg = "L2";
...
function initiateAlg(arrayIndex=0,alg,startAlgorithm = "", speed=400,num=0,highlightBoolean=true) {
    let preventXYZstring = preventXYZ((countXYZ(randAlgArray[arrayIndex] + " " + alg)));
    let startSequence = startAlgorithm == "" ? preventXYZstring + alg : preventXYZstring + startAlgorithm + " " + alg;
    let highlight = highlightBoolean ? algoArrayPrime[arrayIndex][num][0] : "";
    return createConfig(alg,startSequence,speed,num,highlight);
}

function getStartAlg(arrayIndex=0) {
    return [randAlgArray[arrayIndex],partialSolve].join(' ').trim();
}

function loadCurrent(arrayIndex=0,speed=400,num,multipleBoolean=false) {
    testAlg = algoArrayPrime[arrayIndex][[currentNumArray[arrayIndex]]][1];
    //cubeFrame.src = initiateAlg(testAlg,getStartAlg(),speed,num);
    clearAllCanvases();
    removeDivById('cube'+arrayIndex+'a');
    CubeAnimation.create_in_dom(`#parent${arrayIndex}`, initiateAlg(arrayIndex,testAlg,getStartAlg(arrayIndex),speed,num), `id=${'cube'+arrayIndex+'a'} class='cube roofpig mycube noBorder'`);
    let cube = document.getElementById('cube'+arrayIndex+'a');
    cube.setAttribute('data-config',initiateAlg(arrayIndex,testAlg,getStartAlg(arrayIndex),speed,num));
    if(multipleBoolean) {
        let highlightBoolean = false;
        removeDivById('cube'+arrayIndex+'b');
        CubeAnimation.create_in_dom(`#parent${arrayIndex}`, initiateAlg(arrayIndex,testAlg,getStartAlg(arrayIndex),speed,num,highlightBoolean), `id=${'cube'+arrayIndex+'b'} class='cube transparent roofpig mycube noBorder'`);
        cube = document.getElementById('cube'+arrayIndex+'b')
        cube.setAttribute('data-config',initiateAlg(arrayIndex,testAlg,getStartAlg(arrayIndex),speed,num,highlightBoolean));  
    }
    updateButtonTracker(arrayIndex);
}
...
let currentNumArray = [0,0,0,0];
let currentMoveArray = [0,0,0,0];

testAlg = algoArrayPrime[0][0][1];
partialSolve = "";
setTimeout(()=>{
    initialButtonTracker();
    loadCurrent(0,200,0,true);
    loadCurrent(1,200,0,true);
    loadCurrent(2,200,0,true);
    loadCurrent(3,200,0,true);
},500)
...
function nextAction(arrayIndex=0) {
    let currentNum = currentNumArray[arrayIndex];
    let currentMove = currentMoveArray[arrayIndex];
    let moves = algoArrayPrime[arrayIndex][currentNum][1].split(' ');
    if(currentMove > moves.length-1) {
        currentNumArray[arrayIndex] = currentNum + 1;
        currentMoveArray[arrayIndex] = 0;
        testAlg = algoArrayPrime[arrayIndex][currentNum][1];
        partialSolve = concatAlgoArrayUpTo(currentNumArray[arrayIndex],algoArrayPrime[arrayIndex]);
        loadCurrent(arrayIndex,100,currentNumArray[arrayIndex],true);
    } else {
        pressNextButton(arrayIndex);
        currentMoveArray[arrayIndex] = currentMove + 1;
    }
    deleteHelp();
}

function pressNextButton(arrayIndex=0,id="") {
    if(id=="") {

        const elements = document.querySelectorAll('[id*="next-"]');
        for(let element of elements) {
            if(buttonTracker[arrayIndex].includes(element)) {
                element.click();
            }
        }
        //elements[0].click();
...
function initialButtonTracker() {
    const elements = document.querySelectorAll('[id*="next-"]');
    for(let i=0;i<elements.length;i++) {
        if(i<2) {
            buttonTracker[0].push(elements[i]);
        } else if(i<4) {
            buttonTracker[1].push(elements[i]);
        } else if(i<6) {
            buttonTracker[2].push(elements[i]);
        } else if(i<8) {
            buttonTracker[3].push(elements[i]);
        } else {
...
function updateButtonTracker(arrayIndex=0) {
    let oldButtonTracker = buttonTracker[arrayIndex];
    let newButtonTracker = [];
    const elements = document.querySelectorAll('[id*="next-"]');
    console.log(elements)
    for(let element of elements) {
        if(!buttonTracker.flat().includes(element)) {
            newButtonTracker.push(element);
        }
    }
    console.log(newButtonTracker)
    buttonTracker[arrayIndex] = newButtonTracker;
}

function concatAlgoArrayUpTo(number,array=algoArrayPrime[0]) {
    let string = ""
